...
jQuery(function($) {
  $("#rss-feeds").rss("http://bloc.realitat.cat/feeds/posts/default",
  {
  	limit: 10,
  	dateFormat: 'D [de] MM',
	entryTemplate:'<article><header><h3><a href="{url}">{title}</a></h3><h6>{date}</h6></header><div>{teaserImage}{shortBodyPlain}<br><a href="{url}">M&eacute;s informaci&oacute; &gt;&gt;</a></div></article>'
  })
})